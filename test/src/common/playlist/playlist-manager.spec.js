...
describe('PlaylistManager', function() {
  let kontorolPlayer, playlistManager, sandbox;
  const config = {
    ui: {},
    provider: {},
    playback: {
  ...
  before(function() {
    ...
    kontorolPlayer = new KontorolPlayer(config);
  });

  beforeEach(function() {
    playlistManager = new PlaylistManager(kontorolPlayer, config);
  });

  afterEach(function() {
    playlistManager.reset();
    playlistManager = null;
    kontorolPlayer._eventManager.removeAll();
  ...
  describe('configure', function() {
    ...
    it('should load playlist by config and fire event', function() {
      kontorolPlayer._eventManager.listen(kontorolPlayer, kontorolPlayer.Event.Playlist.PLAYLIST_LOADED, event => {
    ...
    it('should not load playlist by config and fire event is no items', function(done) {
      kontorolPlayer._eventManager.listen(kontorolPlayer, kontorolPlayer.Event.Playlist.PLAYLIST_LOADED, () => {
        done(new Error('fail'));
      });
      playlistManager.configure();
      done();
    ...
    it('should play the first item', function(done) {
      kontorolPlayer._eventManager.listen(kontorolPlayer, kontorolPlayer.Event.Playlist.PLAYLIST_ITEM_CHANGED, () => {
        done();
      });
      playlistManager.configure(PlaylistMockData.playlistByConfig);
    });
    ...
    it('should play next on ended when auto continue is true', function(done) {
      ...
      kontorolPlayer._eventManager.listen(kontorolPlayer, kontorolPlayer.Event.Playlist.PLAYLIST_ITEM_CHANGED, () => {
        ...
        kontorolPlayer.dispatchEvent(new FakeEvent(kontorolPlayer.Event.Core.PLAYBACK_ENDED));
      });
      playlistManager.configure(PlaylistMockData.playlistByConfig);
    });
    ...
    it('should fire playlist ended event', function(done) {
      ...
      kontorolPlayer._eventManager.listen(kontorolPlayer, kontorolPlayer.Event.Playlist.PLAYLIST_ITEM_CHANGED, onItemChanged);
      kontorolPlayer._eventManager.listen(kontorolPlayer, kontorolPlayer.Event.Playlist.PLAYLIST_ENDED, () => {
        kontorolPlayer._eventManager.unlisten(kontorolPlayer, kontorolPlayer.Event.Playlist.PLAYLIST_ITEM_CHANGED, onItemChanged);
        done();
      });
      playlistManager.configure(PlaylistMockData.playlistByConfig);
    });
    ...
    it('should play in loop when loop is true', function(done) {
      ...
      kontorolPlayer._eventManager.listen(kontorolPlayer, kontorolPlayer.Event.Playlist.PLAYLIST_ENDED, () => {
        kontorolPlayer._eventManager.unlisten(kontorolPlayer, kontorolPlayer.Event.Playlist.PLAYLIST_ITEM_CHANGED, onItemChanged);
        kontorolPlayer._eventManager.listen(kontorolPlayer, kontorolPlayer.Event.Playlist.PLAYLIST_ITEM_CHANGED, e => {
  ...
  describe('load', function() {
    it('should merge the playlist data and config', function() {
      ...
      playlistManager._playlist.poster.should.equal('http://cdntesting.qa.mkontorol.com/p/1091/sp/0/thumbnail/entry_id/0_wckoqjnn/version/100162');
  ...
  describe('playNext', function() {
    before(function() {
      sinon.stub(kontorolPlayer, 'loadMedia').callsFake(function({entryId}) {
    ...
    it('should call playNext automatically once the playlist loaded', function(done) {
      kontorolPlayer._eventManager.listen(kontorolPlayer, PlaylistEventType.PLAYLIST_ITEM_CHANGED, () => {
        done();
      });
    });
